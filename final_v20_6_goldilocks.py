# CODE: final_v20_6_goldilocks.py - GOLDILOCKS between V20.4 PF3.22 and V20.5 PF0.65
import pandas as pd, yfinance as yf, time
CODE_NAME = "final_v20_6_goldilocks.py"
...
for _ in range(3):
    try:
        ...
        if len(tmp)>500:
            ...
            print(f"[{CODE_NAME}] download ok rows={len(tmp)}")
            break
    except Exception as e:
        print(f"[{CODE_NAME}] retry {e}")
        time.sleep(2)

if len(df)<300:
    raise SystemExit(f"[{CODE_NAME}] rows {len(df)} <300")
...
for i in range(200, len(df)):
    ...
    vol_ok = atr_i > float(atr_sma.iloc[i])*0.95 if not pd.isna(atr_sma.iloc[i]) else True
    ...
    bos_ok = float(close.iloc[i]) > float(high.iloc[i-48:i-1].max()) if i>=48 else False
    retest_ok = abs(float(close.iloc[i])-float(low.iloc[i-2])) < atr_i*0.6 if bull_ob else abs(float(close.iloc[i])-(float(high.iloc[i-2])+float(low.iloc[i]))/2) < atr_i*0.6 if bull_fvg else False
    long_cond = h4_bull and m15_bull and (bull_fvg or bull_ob) and atr_i>0.85*2.1 and vol_ok and bos_ok and retest_ok and (i-last_sig>12)
    if h4_bull and (bull_fvg or bull_ob): valid_buy+=1
    if not in_pos and long_cond:
        in_pos=True; entry=float(close.iloc[i]); sl=entry-atr_i*1.1; tp=entry+atr_i*2.2; bar_entry=i; last_sig=i
    elif in_pos:
